            ...
            <div class="card-body p-4">
                ...
                <div class="table-responsive">
                    <table id="paymentTable" class="table table-striped table-hover" style="width:100%">
                        <thead class="table-primary text-center"  style="font-size: 12px">
                            <tr>
                                <th>Date</th>
                                <th>Payment OR</th>
                                <th>Name</th>
                        ...
                        <tbody  style="font-size: 12px">
                            @if($payment->count() > 0)
                                ...
                                <tr>
                                    <td>{{ $pymnt->paymentDate ?? '' }}</td>
                                    <td>{{ $pymnt->paymentOR ?? '' }}</td>
                                    <td>{{ $pymnt->studentFullname ?? '' }}</td>
                                    <td>{{ number_format($pymnt->balance, 2) ?? '' }}</td>
                                    <td>{{ number_format($pymnt->paymentTuitionAmount, 2) ?? '' }}</td>
                                    <td>{{ number_format($pymnt->paymentAmount, 2) ?? '' }}</td>
                                    <td>{{ $pymnt->paymentPeriod ?? '' }}</td>
                                    <td>
